...
function toWeatherScore(tempF: number, precipProb: number, weatherCode: number): number {
  ...
  if (tempF <= 32) tempScore = 40;
  else if (tempF <= 50) tempScore = 78;
  else if (tempF <= 65) tempScore = 68;
  else if (tempF <= 78) tempScore = 52;
  else if (tempF <= 90) tempScore = 42;
  else tempScore = 30;
...
export async function getWeatherSignal(lat: number, lng: number): Promise<WeatherSignal> {
  ...
  if (!apiKey) {
    return {
      score: 58,
      source: "fallback",
      raw: { lat, lng, error: "TOMORROW_IO_API_KEY not configured" }
    };
  }
  ...
  try {
    ...
    if (!v) {
      return { score: 58, source: "tomorrow", raw: data as Record<string, unknown> };
    }
