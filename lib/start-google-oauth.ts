...
export async function startGoogleOAuth(
  ...
  const popup = window.open(
    ...
    `popup=yes,width=${w},height=${h},left=${left},top=${top},resizable=yes,scrollbars=yes,noreferrer`
  );
  ...
  if (!popup) {
    ...
    if (fallback.error) {
      return { error: formatAuthError(fallback.error) };
    }
    if (fallback.data?.url) {
      window.location.assign(fallback.data.url);
      return "success";
    }
    return { error: "Could not start Google sign-in (popup blocked)." };
  }
  ...
  return await new Promise<GoogleOAuthOutcome>((resolve) => {
    ...
    const onMessage = (ev: MessageEvent) => {
      ...
      if (!d.ok) {
        finish({ error: typeof d.error === "string" && d.error.trim() ? d.error : "Google sign-in failed" });
        return;
      }
    ...
    const poll = window.setInterval(() => {
      void (async () => {
        ...
        if (popup.closed) {
          closedTicks += 1;
          // Brief delay so session cookies from the popup are visible in the opener.
          if (closedTicks >= 5) {
            finish("cancelled");
          }
        } else {
